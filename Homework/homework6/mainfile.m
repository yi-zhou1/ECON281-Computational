%% ECON281 Computation Homework 6
% This aims to modify the code in class by using Homework 5 code solving
% the Aiyagari model
% I tried to replace "compute_steady_state" with Week 5 Aiyagari code and
% modify "equilibrium_conditions". Right now the "compute_steady_state" can
% be replaced but there is error about myAD function when reaching to equilibrium
% conditions
% Author: Yi Zhou
% Date: 05/13/2024
...
addpath('/Users/yizhou/Library/CloudStorage/Dropbox/PhD2/ECON281Comp/MATLABAutoDiff-master');
addpath('/Users/yizhou/Library/CloudStorage/Dropbox/PhD2/ECON281Comp/phact-master');
...
global IfSS IbSS I0SS varsSS A l_supply

[rSS,wSS,KSS,ASS,uSS,cSS,VSS,gSS,dVUSS,dVfSS,dVbSS,IfSS,IbSS,I0SS,l_supply] = ...
    aiyagri_steady_state();   % instead of running compute_steady_state, run aiyagari code from week 5
...
varsSS(4*I+2,1) = rSS;
varsSS(4*I+3,1) = wSS;
varsSS(4*I+4,1) = (KSS ^ param.alpha) * (l_supply ^ (1 - param.alpha));
CSS = sum(cSS(:) .* gSS(:) * grid.da);
varsSS(4*I+5,1) = CSS;
varsSS(4*I+6,1) = param.delta * KSS;

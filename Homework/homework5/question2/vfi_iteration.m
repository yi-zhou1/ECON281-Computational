function [v_new,c,A,saving] = vfi_iteration(v0,param,num,grid,r,w)
...
[Va_Upwind,ssb,ssf] = vp_upwind(v0,param,num,grid,r,w) ;

c = Va_Upwind.^(-1);
saving = (1+r.*param.rshock).*grid.a + w.*param.e - c;
u = utility(c);
X = - min(ssb,0)/grid.da;

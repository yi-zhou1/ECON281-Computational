function [Va_Upwind,sb,sf] = vp_upwind(v0,param,num,grid,r,w)
...
Vab(1,:) = (r*param.rshock.*grid.a(1,:) + w.*param.e).^(-1); %state constraint boundary condition    


%consumption and savings with forward difference
cf = max(Vaf,1e-08).^(-1);
sf = w.*param.e + r*param.rshock.*grid.a - cf ;
%consumption and savings with backward difference
cb = max(Vab,1e-08).^(-1);
sb = w.*param.e + r*param.rshock.*grid.a - cb ;
%consumption and derivative of value function at steady state
c0 = w.*param.e + r*param.rshock.*grid.a ;
Va0 = c0.^(-1);

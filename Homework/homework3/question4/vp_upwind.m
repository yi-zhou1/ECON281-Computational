...
Vaf(end) = 0; 
Vab(1) = (param.r*grid.a(1) + pi(1)).^(-1); %state constraint boundary condition that makes things stay in the state space 


%consumption and savings with forward difference
cf = max(Vaf,1e-08).^(-1);   % this is important
sf = param.r .* grid.a + pi - cf ;


%consumption and savings with backward difference
cb = max(Vab,1e-08).^(-1); % to avoid negative consumption
sb = param.r .* grid.a + pi - cb ;
%consumption and derivative of value function at steady state
c0 = pi + param.r .* grid.a ;
Va0 = c0.^(-1);
